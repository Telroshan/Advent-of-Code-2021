...
func main() {
	...
	numberCounts := make([]int, 10)

	for _, line := range fileLines {
		split := strings.Split(line, " | ")
		outputSignals := strings.Split(split[1], " ")
		for _, outputSignal := range outputSignals {
			if len(outputSignal) == 2 {
				numberCounts[1]++
			} else if len(outputSignal) == 4 {
				numberCounts[4]++
			} else if len(outputSignal) == 3 {
				numberCounts[7]++
			} else if len(outputSignal) == 7 {
				numberCounts[8]++
			}
		}
	}

	fmt.Println(numberCounts, "=>", numberCounts[1]+numberCounts[4]+numberCounts[7]+numberCounts[8], "instances of 1, 4, 7 and 8")

	totalOutput := 0
	for _, line := range fileLines {
		split := strings.Split(line, " | ")
		numberSignals := strings.Split(split[0], " ")

		numberPatterns := make([]string, 10)
		// First loop to determine the basic ones, i.e with unique segments count, aka 1, 4, 7 and 8
		for _, signal := range numberSignals {
			if len(signal) == 2 {
				numberPatterns[1] = signal
			} else if len(signal) == 4 {
				numberPatterns[4] = signal
			} else if len(signal) == 3 {
				numberPatterns[7] = signal
			} else if len(signal) == 7 {
				numberPatterns[8] = signal
			}
		}
		// Determine 5-characters numbers
		for _, signal := range numberSignals {
			// 2, 3, 5 have 5 characters
			if len(signal) != 5 {
				continue
			}
			// Among 2, 3, 5, the one that contains the 2 digits of the 1 is the number 3
			containsNumber1Chars := true
			for _, number1Char := range numberPatterns[1] {
				if !containsChar(signal, number1Char) {
					containsNumber1Chars = false
					break
				}
			}
			if containsNumber1Chars {
				numberPatterns[3] = signal
				continue
			}

			// 5 has 3 common digits with 4, whereas 2 only has 2 common digits
			commonDigitsWith4 := 0
			for _, number4Char := range numberPatterns[4] {
				if containsChar(signal, number4Char) {
					commonDigitsWith4++
				}
			}
			if commonDigitsWith4 == 3 {
				numberPatterns[5] = signal
				continue
			}

			numberPatterns[2] = signal
		}
		...
		for _, signal := range numberSignals {
			...
			// 6 and 9 contain every character of 5, so determine 0 here
			containsEveryNumber5Char := true
			for _, number5Char := range numberPatterns[5] {
				if !containsChar(signal, number5Char) {
					containsEveryNumber5Char = false
					break
				}
			}
			if !containsEveryNumber5Char {
				numberPatterns[0] = signal
				continue
			}

			// 9 contains every character of 3
			containsEveryNumber3Char := true
			for _, number3Char := range numberPatterns[3] {
				if !containsChar(signal, number3Char) {
					containsEveryNumber3Char = false
					break
				}
			}
			if !containsEveryNumber3Char {
				numberPatterns[6] = signal
				continue
			}

			numberPatterns[9] = signal
		}

		outputSignals := strings.Split(split[1], " ")
		outputValue := 0
		multiplier := 1
		for i := 1; i < len(outputSignals); i++ {
			multiplier *= 10
		}
		// fmt.Println("----- ", outputSignals)
		for _, signal := range outputSignals {
			// foundMatchingPattern := false
			for patternIndex, pattern := range numberPatterns {
				if len(signal) != len(pattern) {
					continue
				}
				matches := true
				for _, patternChar := range pattern {
					if !containsChar(signal, patternChar) {
						matches = false
						break
					}
				}
				...
				outputValue += patternIndex * multiplier
				// fmt.Println(outputValue," (", signal,"=>",patternIndex,"=>",patternIndex * multiplier,")")
				multiplier /= 10
				break
			}
		}
